...
| 2.6 | **Módulos de IA por deporte** (especializaciones de `tid_agent.py`) | 🟡 | Decisión E3. Mismo motor fisiológico, distinto system prompt + contexto por deporte; el usuario elige deporte en su perfil. **Orden (roadmap NORTE):** 1) Running/Maratón · 2) Triatlón · 3) Ciclismo · 4) Natación/Aqua (I+D con Gael desde día 1) · 5) Fuerza/CrossFit · 6) Fútbol/Básquet. ✅ **Scaffold + módulo Running CONSTRUIDOS** (`tid_agent.py` sport-aware: `load_sport`/`build_system`/`--deporte`; `deportes/natacion.json` + `deportes/running.json`). El motor no se toca; el módulo solo cambia persona + enfoque + prioridad de contexto. Probado (reglas + selección/override). Falta: siguientes deportes (Triatlón…) + validar Running con dato de corredor (economía/ritmo requiere ingesta GPS). Detalle: `analisis/roadmap-modulos-deporte.md`. |
