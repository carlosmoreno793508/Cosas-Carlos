...
| 2.6 | **Módulos de IA por deporte** (especializaciones de `tid_agent.py`) | 🟡 | Decisión E3. Mismo motor fisiológico, distinto system prompt + contexto por deporte; el usuario elige deporte en su perfil. **Orden (roadmap NORTE):** 1) Running/Maratón · 2) Triatlón · 3) Ciclismo · 4) Natación/Aqua (I+D con Gael desde día 1) · 5) Fuerza/CrossFit · 6) Fútbol/Básquet. ✅ **Scaffold + 4 módulos CONSTRUIDOS** (`tid_agent.py` sport-aware: `load_sport`/`build_system`/`--deporte`; `deportes/`: **natacion, running, triatlon, ciclismo** = el trío de resistencia + natación). El motor no se toca; el módulo solo cambia persona + enfoque + prioridad de contexto. Probado (carga de los 4, reglas + selección/override). Falta: Fuerza/CrossFit y Fútbol/Básquet; validar con dato real de cada deporte (economía/potencia requiere ingesta GPS/power). Detalle: `analisis/roadmap-modulos-deporte.md`. |
...
| H0.2 | Enviar RFQ (correo NDA-first) a ODMs | 🟡 | **CORREOS ENVIADOS por Carlos (2026-08).** Flujo NDA-first (`correo-rfq-fabricas-EN.md`, Rev. B): Correo 1 = scope + NDA (sin adjunto) → NDA → RFQ. **Shortlist por fit** (`checklist-fabricas-rfq.md`): **J-Style/Jointcorp**, **Vositone**, **MOKOSmart**. Filtro clave = **raw PPG/IBI + SDK**. **Respuestas recibidas (2026-08-04):** ⬇️ ver H0.2a/H0.2b. **Riesgo:** solo 1 candidato viable hoy → seguir tocando ODMs para tener poder de negociación. |
| H0.2a | **Vositone** (Jack Ho, sales@vositone.com) — ODM full-link + SDK | 🟡 | **CANDIDATO ACTIVO.** Confirma raw PPG ≥100 Hz + IBI/RR + IMU vía **SDK propietario**; acepta Mutual NDA; ODM full-link (NRE bajo, plataforma reusable). **🚩 Bandera:** su SDK exige **pago completo del fee ANTES** de validar/ver dato ("no free trial/partial/temporary access"). **Contra-respuesta enviada (`correo-rfq-fabricas-EN.md` Correo 3):** NDA primero → **muestra/spec del SDK antes de pagar** → **costo del SDK** + qué incluye → **costo prototipos/EVK + NRE + MOQ** → fee **acreditable/escrow/reembolsable**. Borrador Gmail creado. **En espera de respuesta.** |
| H0.2b | **Nicholas Xu** (Sales Engineer) — EMS/CM puro | ⬜ (Fase 2) | **NO encaja etapa actual:** "we do not offer design/development, only manufacturing after designs ready". Es contract manufacturer, **sin plataforma/SDK/raw data**. **Archivado para Fase de producción** (fabricar diseño propio ya maduro). Correo de cortesía enviado (borrador Gmail). Recontactar en H-producción. |
